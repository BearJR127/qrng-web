...
async function runGameLogic(){
  ...
  function updateChart(snapshot) {
    ...
    chartInstance = new Chart(ctxc, {
      type:'bar',
      // Colors correspond to Red, Blue, Green, Yellow, Black, White and All
  ...
  function updateAnalysis(){
    ...
    analysisChart=new Chart(ctxa,{type:'bar',
      // Colors correspond to Red, Blue, Green, Yellow, Black, White, All RBGY, All BW
      data:{labels,datasets:[{label:'Match %',data:datasetData,backgroundColor:['red','blue','green','yellow','black','#ccc','#888','#444']}]},
      options:{scales:{y:{beginAtZero:true,max:100}}}});
